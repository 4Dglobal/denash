<?php
 class empdetailsModel extends CI_Model
 {
    ...
    if($userdata['role'] == 'agent'){
      ...
      if($val == ''){
        $q = $this->db->query("SELECT * FROM emp_personal_details limit $start,$limit");
      }
      else{
        $q = $this->db->query("SELECT * FROM emp_personal_details where Emp_id like '%".$val."%' or Emp_name like '%".$val."%' limit $start,$limit");
      }
    }
  ...
  public function agentdata(){
    $user =$this->db->query("SELECT * FROM users WHERE role!='admin' and emp_id not in (SELECT Emp_id FROM emp_personal_details)");
    return $user->result();
  }
...
//print_r($data);
    ...
    $values = array(
            ...
            "Education_study" => $data['EStudy'],
            "Education_institute" => $data['ESchool'],
            "Education_year" => $data['EMark'],
            "Family_name" => $data['FName'],
            "Family_relationship" => $data['FRelationship'],
            "Family_contact" => $data['FContact'],
            "DOJ" => $data['doj'],
            "Work_email" => $data['workemail'],
            "Process" => $data['process'],
            "Exp_in_MB" => $data['expinMB'],
            "First_MB_Employer" => $data['firstMBEmployee'],
            "Start_Date_WithFirst_MB_Employer" => $data['StartDate_FirstMB'],
            "Other_Past_MB_Employers" => $data['OtherPastMB'],
            "MB_Softwares_Worked_On" => $data['MBSoftwareon'],
            "MB_Specialties_Worked_On" => $data['MBSpecial'],
            "MB_Processed_Worked_On" => $data['MBProcessWork'],
  ...
  public function updateDetails($data){
    ...
    $values = array(
            ...
            "MB_Softwares_Worked_On" => $data['UpdateMBSoftwareon'],
            "MB_Specialties_Worked_On" => $data['UpdateMBSpecial'],
